# Initial review of the existing research sources

10 September 2026 · Abstract and source-summary review

This review covers the 11 distinct arXiv papers linked in [Perspectives](../notes/PERSPECTIVES.md), its two research essays, and the two methods essays linked in the [README](../README.md). The [Construct synthesis](../notes/RESEARCH_BRIEF.md) supplies the local evidence. The [ancillary-study manuscript](../notes/ANCILLARY_STUDY.md) supplies organizational context. arXiv access and submission documentation are operational references, outside this scientific reading list.

All 11 paper abstracts and their metadata were retrieved together through the arXiv query API. The exact response is [cached here](2026-09-10/arxiv-query.xml); versioned links below identify the abstract records reviewed. Paper bodies, code, and experimental artifacts have not been systematically reviewed. The production paper's linked weight-tuning section received an additional targeted check. Web essays were read at their linked URLs on this date; they have no arXiv version identifier.

The summaries below report authors' claims. The limitations and questions are our assessment of what this reading establishes. Abstract-level review can identify overlap and useful questions, but cannot establish that a question is unanswered across the literature. The resulting [theory and question map](THEORIES_AND_QUESTIONS.md) is provisional.
...
## What this first pass changes

The existing sources already cover textual reflection, executable skill reuse, evolving playbooks, neural memory, online updates, and training memory behavior. A broad claim that agents can improve through persistent state would overlap heavily with this work.

Our synthesis instead motivates questions about the conditions under which a representation remains useful: access, transfer, correction, recurrence cost, authority, and composition. These are directions to investigate, with novelty still to be checked. The most useful next reading will inspect the closest methods and ablations for a selected question, then widen the literature search where this initial bibliography is thin.

**Research preference after this review:** Prioritize neural memory, live weight updates, and learned memory policies. Voyager remains relevant background on system-level persistence, but executable skill libraries are a lower-interest direction for this program. The question map reflects this preference without treating it as an empirical verdict about executable skills.
